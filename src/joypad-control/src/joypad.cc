...
const std::unordered_map<uint8_t, cstring_t> Joypad::triggers_{
    {6, trigger::KL2}, {7, trigger::KR2}};

const std::unordered_map<uint8_t, cstring_t> Joypad::thumbsticks_{
    {0, thumbstick::k3},
    {1, thumbstick::kL3},
    {3, thumbstick::kR3},
    {4, thumbstick::kR3}};

const std::unordered_map<uint8_t, cstring_t> Joypad::buttons_{
...
const std::unordered_map<uint8_t, cstring_t> Joypad::special_{
    {8, button::kSelect},
    {9, button::kStart},
...
Joypad::Joypad() {
  ...
  joy_sub_ = node_handler_.subscribe<sensor_msgs::Joy>(
      "joy", 1000, &Joypad::controllerCallback, this);
}
